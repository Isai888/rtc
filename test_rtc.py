...
def test_user_with_invalid_password(selenium):
    ...
    result = page.enter_login_or_password(selenium, page.personal_account, page.password)  # ввод лиц. счета и пароля
...
def test_link_color_forgot_password(selenium):
    ...
    page = AuthPage(selenium)
    page.tab_ls.click()  # активировали таб Лицевой счет
    page.enter_login_or_password(selenium, page.personal_account, page.password)  # ввод лицевого счета и пароля
    assert "rt-link--orange" in page.forget_password_link.get_attribute(
        'class'), "В соответствии с ТЗ цвет ссылки должен был измениться "
...
@pytest.mark.parametrize("input, description, expected_output",
                         [
                             ...
                             ("pythonpython", "Латиницей без заглавной",
